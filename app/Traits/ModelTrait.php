...
trait ModelTrait
{
    ...
    public function scopeSearch($query, $options)
    {
        ...
        $rawFields = array_key_exists('rawFields', $options) ? $options['rawFields'] : [];
        // Default to using the model's searchable fields if not provided
        $fields = is_array($fields) ? $fields : ($this->searchable ?? []);

        // Return the query as is if no search term is provided or if both fields and rawFields are empty
        if (empty($search) || (empty($fields) && empty($rawFields))) {
            return $query;
        }
    ...
    public function scopeisTrash($query, $isTrash = null)
    {
        if (is_null($isTrash)) {
            return $query;
        }

        return $isTrash ? $query->whereIn('status', ['Archived', 'Deactivated']) : $query->where('status', 'Active');
    }
}
